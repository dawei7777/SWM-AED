...
def yoao(image, tlabel, net, num_classes, overshoot, max_iter, margin_factor=0):
    image = image.cuda()
    net = net.cuda()
    ...
    while k_i == tlabel and loop_i < max_iter:

        pert = np.inf
        fs[0, tlabel].backward(retain_graph=True)
        #     对正确标签执行反向传播
        ...
        #     当前类别和正确类别之间的梯度差
        f_k = (fs[0, I[max_grad_norm_idx]] - fs[0, tlabel]).data.cpu().numpy()
        #     当前类别和正确类别之间的置信度差
    ...
    pert_image = pert_image.type(torch.FloatTensor).cuda()
    x = Variable(pert_image, requires_grad=True)
    #     图像转换为变量，设置其需要梯度
    fs = net.forward(x)
    k_i = np.argmax(fs.data.cpu().numpy().flatten())
